      ...
      <el-row :gutter="10">
        ...
        <el-col :span="8">
          <el-form-item label="比赛密码" prop="password">
            <el-input v-model="contest.password" placeholder="比赛密码,不填写则无密码"/>
          </el-form-item>
        </el-col>
      </el-row>
      <el-row :gutter="10">
        <el-col :span="8">
          <el-form-item label="比赛规则" prop="ruleType">
            <el-radio-group v-model="contest.ruleType">
              <el-radio v-for="item in $config.supportRuleTypes"
                        :label="item.value"
                        :key="item.title"
        ...
        <el-col :span="4" v-if="contest.ruleType===1">
  ...
  data() {
    ...
    let validateEndTime = (rule, value, callback) => {
      if (value === '' || value === null) {
        ...
        let end = this.$dayjs(value)
        let start = this.$dayjs(this.contest.startTime)
    ...
    handleSave() {
      this.$refs['contest'].validate(valid => {
        if (valid) {
          ...
          contest.startTime = this.$dayjs(contest.startTime).format('YYYY-MM-DD HH:mm:ss')
          contest.endTime = this.$dayjs(contest.endTime).format('YYYY-MM-DD HH:mm:ss')
          console.log(contest);
          if (this.contestId !== -1){
            updateContest(contest).then(res=>{
              if (res.success){
